/*
 * Vapi API
 *
 * Voice AI for developers.
 *
 * The version of the OpenAPI document: 1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ClientMessageToolCallsToolWithToolCallListInner {
    FunctionToolWithToolCall(Box<models::FunctionToolWithToolCall>),
    GhlToolWithToolCall(Box<models::GhlToolWithToolCall>),
    MakeToolWithToolCall(Box<models::MakeToolWithToolCall>),
    BashToolWithToolCall(Box<models::BashToolWithToolCall>),
    ComputerToolWithToolCall(Box<models::ComputerToolWithToolCall>),
    TextEditorToolWithToolCall(Box<models::TextEditorToolWithToolCall>),
}

impl Default for ClientMessageToolCallsToolWithToolCallListInner {
    fn default() -> Self {
        Self::FunctionToolWithToolCall(Default::default())
    }
}
/// The type of tool. \"function\" for Function tool.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "function")]
    Function,
    #[serde(rename = "ghl")]
    Ghl,
    #[serde(rename = "make")]
    Make,
    #[serde(rename = "bash")]
    Bash,
    #[serde(rename = "computer")]
    Computer,
    #[serde(rename = "textEditor")]
    TextEditor,
}

impl Default for Type {
    fn default() -> Type {
        Self::Function
    }
}
/// The sub type of tool.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum SubType {
    #[serde(rename = "text_editor_20241022")]
    TextEditor20241022,
}

impl Default for SubType {
    fn default() -> SubType {
        Self::TextEditor20241022
    }
}
/// The name of the tool, fixed to 'str_replace_editor'
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Name {
    #[serde(rename = "str_replace_editor")]
    StrReplaceEditor,
}

impl Default for Name {
    fn default() -> Name {
        Self::StrReplaceEditor
    }
}

