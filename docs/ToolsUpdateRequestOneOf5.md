...
**messages** | Option<[**Vec<models::UpdateTransferCallToolDtoMessagesItem>**](UpdateTransferCallToolDtoMessagesItem.md)> | These are the messages that will be spoken to the user as the tool is running.  For some tools, this is auto-filled based on special fields like `tool.destinations`. For others like the function tool, these can be custom configured. | [optional]
